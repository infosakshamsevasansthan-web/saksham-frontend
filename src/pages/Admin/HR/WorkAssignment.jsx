...
// --- Corrected Imports (Matching your GitHub Screenshot) ---
...
const WorkAssignment = () => {
  ...
  const handleAction = (member) => {
    ...
    else toast.error("Is post ke liye assignment set nahi hai");
  };
    ...
    <CityLayout>
      ...
      <div className="p-4 md:p-8 space-y-6 bg-[#f8fafc] min-h-screen text-left">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 flex justify-between items-center">
           ...
           <button onClick={fetchData} className="p-4 bg-slate-50 text-slate-500 rounded-2xl hover:bg-indigo-50 active:scale-95 transition-all">
        ...
        <div className="bg-white p-5 rounded-[2rem] shadow-sm flex gap-4">
           <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search by name or employee ID..." className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl font-bold text-sm outline-none focus:ring-2 ring-indigo-500/20" onChange={(e) => setSearchTerm(e.target.value)} />
           </div>
           <button className="px-8 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-colors"><FileText size={18} className="inline mr-2"/> Export</button>
        </div>
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden">
           {loading ? (
             <div className="p-20 text-center flex flex-col items-center gap-4">
                ...
                <p className="font-bold text-slate-400 uppercase text-xs tracking-widest">Loading Personnel Data...</p>
             </div>
           ) : (
                    ...
                    <tr key={s.id} className="hover:bg-indigo-50/50 transition-all group">
                       ...
                       <td className="p-6">
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">{s.is_assigned ? 'Allocated' : 'Unassigned'}</span>
                          </div>
                       </td>
