...
import { UserPlus, Search, Users, X, Save, Loader2, Sparkles, Edit3, Download, UploadCloud } from 'lucide-react';
...
const StaffList = () => {
    ...
    // 2. Fetch Next Auto-ID (Sirf Registration ke waqt)
    ...
    useEffect(() => {
        if (showModal && !isEditMode) generateNextId();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const toastId = toast.loading(isEditMode ? "Updating..." : "Registering...");
        try {
            ...
            if (res.data.success) {
                toast.success(isEditMode ? "Staff Updated!" : "Staff Registered!", { id: toastId });
                closeModal();
                fetchStaff();
            }
    ...
    const handleBulkUpload = async (e) => {
        ...
        try {
            ...
            if (res.data.success) { toast.success(res.data.message, { id: toastId }); fetchStaff(); }
        } catch (err) { toast.error("Failed", { id: toastId }); }
    };
            ...
            <div className="p-4 space-y-6 text-left animate-in fade-in duration-500">
                ...
                <header className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[35px] border border-slate-100 shadow-sm gap-4">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={downloadTemplate} className="bg-slate-50 text-slate-600 px-5 py-3 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 hover:bg-slate-200 transition-all">
                            <Download size={16} /> Template
                        </button>
                        <button onClick={() => fileInputRef.current.click()} className="bg-blue-50 text-blue-600 px-5 py-3 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 hover:bg-blue-600 hover:text-white transition-all">
                            <UploadCloud size={16} /> Bulk Upload
                        </button>
                        <button onClick={() => { setIsEditMode(false); setShowModal(true); }} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 hover:bg-emerald-600 transition-all shadow-xl">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-2xl rounded-[45px] shadow-2xl overflow-hidden border border-slate-100">
                            <div className="p-8 border-b border-slate-50 bg-slate-50 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-emerald-600 font-bold text-[10px] uppercase tracking-widest mt-2 flex items-center gap-2"><Sparkles size={12}/> {isEditMode ? 'Editing' : 'Auto ID'}: {formData.employee_id}</p>
                                </div>
                                <button onClick={closeModal} className="w-11 h-11 bg-white rounded-xl text-rose-500 font-black shadow-sm hover:bg-rose-50 transition-all">X</button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    ...
                                    <input required value={formData.full_name_en} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500/20 rounded-2xl outline-none font-bold text-sm uppercase transition-all" onChange={(e) => handleTranslation(e.target.value)} />
                                </div>
                                <div className="space-y-1">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input required value={formData.fh_name} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500/20 rounded-2xl outline-none font-bold text-sm uppercase" onChange={(e)=>setFormData({...formData, fh_name: e.target.value})} />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[9px] font-black text-slate-400 uppercase ml-2">Gender & Mobile</label>
                                    <div className="flex gap-2">
                                        <select value={formData.gender} className="p-4 bg-slate-50 border-none rounded-2xl font-bold text-xs outline-none shadow-inner" onChange={(e)=>setFormData({...formData, gender: e.target.value})}>
                                            <option>Male</option><option>Female</option><option>Other</option>
                                        </select>
                                        <input required value={formData.mobile} className="flex-1 p-4 bg-slate-50 border-none rounded-2xl font-bold text-xs shadow-inner" placeholder="10 Digit Mobile" onChange={(e)=>setFormData({...formData, mobile: e.target.value})} />
                                    </div>
                                </div>
                                <div className="col-span-1 md:col-span-2 space-y-1">
                                    <label className="text-[9px] font-black text-slate-400 uppercase ml-2">Current Address</label>
                                    <input required value={formData.address} className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold text-xs shadow-inner" onChange={(e)=>setFormData({...formData, address: e.target.value})} />
                                </div>
