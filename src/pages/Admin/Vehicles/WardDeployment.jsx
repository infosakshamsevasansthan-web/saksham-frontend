...
import { Truck, MapPin, User, Plus, X, CheckCircle, RefreshCcw, Loader2, RotateCcw, Clock, Calendar } from 'lucide-react';
...
const WardDeployment = () => {
    ...
    // 🟢 Ab sirf gaadi aur ward ki state chahiye
    ...
    const fetchData = async () => {
        ...
        try {
            const [depRes, vhlRes, wrdRes] = await Promise.all([
                axios.get(`https://saksham-backend-9719.onrender.com/api/admin/dispatch/today/${tenantId}`),
    ...
    const handleAssign = async (e) => {
        ...
        // 🟢 Logic: Selected vehicle se uska assigned driver/helper dhoondo
        ...
        try {
            const res = await axios.post('https://saksham-backend-9719.onrender.com/api/admin/deployments/assign', {
                ...
                // 🟢 Ye database se gaadi ke saath jude hue IDs hain
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {loading ? (
                                ...
                                <tr key={d.id} className="hover:bg-slate-50/80 transition-all group">
                                    ...
                                    <td className="p-4">
                                        <p className="text-slate-800 font-bold text-xs">{d.dr_en} / {d.dr_hi}</p>
                                    ...
                                    <td className="p-4 text-center">
                                        {d.status === 'on-duty' && (
                                            <button onClick={() => markReturned(d.id)} className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm"><RotateCcw size={16}/></button>
                                        )}
                                    </td>
                                </tr>
                            ))}
                    ...
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-lg rounded-[50px] shadow-2xl overflow-hidden">
                            <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                                ...
                                <button onClick={() => setShowModal(false)} className="p-2 text-rose-500">X</button>
                            </div>
